"""Bundled ``tree_sitter_groovy`` grammar.
...
tree-sitter-groovy grammar uses for them. See ``tree_sitter/_brace.py`` for the
recall-versus-correctness trade this makes, and ``tree_sitter/_langs.py`` for
this language's node type names.

If the real ``tree_sitter_groovy`` wheel is installed, the front-end delegates to it and none
of this is used.
"""
...
def language() -> PureGrammar:
    """Grammar handle, as the real ``tree_sitter_groovy`` exposes it."""
    spec = SPECS["groovy"]
    return PureGrammar("groovy", lambda source: parse_brace_language(source, spec))
