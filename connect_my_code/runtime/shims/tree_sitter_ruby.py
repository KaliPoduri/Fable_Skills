"""Bundled ``tree_sitter_ruby`` grammar.
...
tree-sitter-ruby grammar uses for them. See ``tree_sitter/_brace.py`` for the
recall-versus-correctness trade this makes, and ``tree_sitter/_langs.py`` for
this language's node type names.

If the real ``tree_sitter_ruby`` wheel is installed, the front-end delegates to it and none
of this is used.
"""
...
def language() -> PureGrammar:
    """Grammar handle, as the real ``tree_sitter_ruby`` exposes it."""
    spec = SPECS["ruby"]
    return PureGrammar("ruby", lambda source: parse_brace_language(source, spec))
