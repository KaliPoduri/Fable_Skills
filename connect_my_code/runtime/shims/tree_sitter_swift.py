"""Bundled ``tree_sitter_swift`` grammar.
...
tree-sitter-swift grammar uses for them. See ``tree_sitter/_brace.py`` for the
recall-versus-correctness trade this makes, and ``tree_sitter/_langs.py`` for
this language's node type names.

If the real ``tree_sitter_swift`` wheel is installed, the front-end delegates to it and none
of this is used.
"""
...
def language() -> PureGrammar:
    """Grammar handle, as the real ``tree_sitter_swift`` exposes it."""
    spec = SPECS["swift"]
    return PureGrammar("swift", lambda source: parse_brace_language(source, spec))
