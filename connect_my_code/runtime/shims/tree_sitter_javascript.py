"""Bundled ``tree_sitter_javascript`` grammar.
...
tree-sitter-javascript grammar uses for them. See ``tree_sitter/_brace.py`` for the
recall-versus-correctness trade this makes, and ``tree_sitter/_langs.py`` for
this language's node type names.

If the real ``tree_sitter_javascript`` wheel is installed, the front-end delegates to it and none
of this is used.
"""
...
def language() -> PureGrammar:
    """Grammar handle, as the real ``tree_sitter_javascript`` exposes it."""
    spec = SPECS["javascript"]
    return PureGrammar("javascript", lambda source: parse_brace_language(source, spec))
