...
1. Go/no-go unknowns were sequenced too late: org policy enabling Copilot Agent Skills and Python availability must be a Phase-0 gate verified with one throwaway skill before any of the library is built.
2. Trigger dilution is worse than stated: 50+ descriptions at up to 1,024 chars exceed every known listing budget (Claude Code ~1% + ~250-char per-description truncation; Codex 2%/8k fallback; Copilot undocumented); curated install bundles and front-loaded descriptions must be a design decision now.
3. The eval plan is not executable on the primary harness: ~20 trigger + ≥3 output cases × 50+ skills has no stated runner; VS Code Copilot Chat cannot batch-test skill selection; specify a real runner or cut counts.
4. No adoption checkpoint before full build-out: ship Tier 1, run a 2–4 week team pilot, let usage decide Tiers 2–3.
5. Copilot Agent Skills "GA" was overstated: VS Code stable arrived early Jan 2026, gh skill is preview; treat as preview surface, pin known-good versions.
6. OWASP Code Review Guide 2.0 (2017) is a stale anchor for security-code-reviewer; use OWASP Top 10:2025 + ASVS 5.0.0 + CWE Top 25 as primaries.
7. Single-generator, single-reviewer quality funnel: skill-creator generating everything concentrates risk; add a hard stop after the first 3–5 generated skills for user review of generator output quality.
8. dependency-auditor without live CVE data is of marginal value; re-scope aggressively or cut.
9. Scrum facilitation skills (sprint-facilitator, retrospective-facilitator, meeting-notes-writer) are weak fits for a coding-agent harness and may not beat raw Copilot — candidates to cut.
10. Codex counts the 1024-char description limit in bytes; cap descriptions well below 1024.
11. Cursor compat-scan and global-skills claims are community-sourced, not official; label them so.
12. gh skill install resolves versions from the latest tagged release; without git tags, installs float on default-branch HEAD — add tagging discipline.
13. LICENSE is low-stakes for a private internal repo; an internal-use notice suffices.
14. The universal hard constraints baked into every skill add ~50 copies of boilerplate; one line per skill plus a shared authoring-guide section is enough.
15. python3 is frequently absent on Windows (it's python or py); CI should test script invocation on Windows, not just lint paths.
