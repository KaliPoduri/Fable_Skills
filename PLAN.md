...
│   └── <skill-name>/
│       ├── SKILL.md
│       ├── README.md          # [CONFIRMED] (user approved) per-skill setup+usage per harness
...
- [CONFIRMED] (user approved) Every skill folder contains README.md: what the skill does, example prompts, and setup + usage for each harness.
...
[CANDIDATE] The full inventory below came from subagent research (July 2026) with source-version verification noted per the register; individual skills await user/council review. 50 skills = 48 lifecycle skills + 2 meta skills.

### Meta (build first — they build the rest)
| Skill | Kind | Purpose |
|---|---|---|
| `skill-creator` | R | [USER-required] Takes user requirements → produces a new skill folder (SKILL.md, README, references, evals) conforming to this library's standards |
| `library-maintainer` | R | [CONFIRMED] (user approved) Re-verifies sources, updates stale content, runs validation; documents review cadence. Web re-verification runs outside the org environment |
...
## 4. Quality & testing plan (success criterion 1 and 2)
...
- [CANDIDATE] Manual acceptance: user smoke-tests Tier-1 skills in VS Code Copilot Chat in the org environment (skills feature enabled?) before mass rollout to the team.
...
- [CANDIDATE] Phase B — Tier 1 (18 skills incl. meta): the core-of-core set above; each skill: research-distilled content (from sources already gathered; any new web research happens outside the org), SKILL.md, README, references, assets, scripts where needed, evals; validate.
...
- [CANDIDATE] R2 Org policy blocks: Copilot Agent Skills could be disabled by org admin settings; Python may be unavailable. Mitigation: [OPEN] items above verified by user in-org before Phase B completes.
...
- [CANDIDATE] R4 Overlap confusion: adjacent skills fire wrongly. Mitigation: boundary sentences in descriptions + near-miss evals (§3).
- [CANDIDATE] R5 Windows/POSIX drift: team members on different OSes; scripts must avoid backslash paths and exec-bit assumptions. Mitigation: authoring standards §2.3 + CI check.
...
- [OPEN] Standards versions the researcher could not verify this session (tagged in UNKNOWNS.md): Conventional Commits 1.0.0, SemVer 2.0.0, Keep a Changelog 1.1.0, ISO 25010:2023, PMBOK 7, OpenSLO version, CWE Top 25 2025, book editions — verify during authoring of the affected skills.
